...
@dataclass(frozen=True, slots=True)
class ScopeView:
    """Sicht eines Scopes (D473 Beschluss 3)."""
...
def scope_view(store: SqliteStore, scope: bytes, now: int) -> ScopeView:
    """Sicht eines Scopes (D473 Beschluss 3, 04 §4.5, 04 §6.2, 03 §3.3.2)."""
    ...
    return ScopeView(state=state, verein=verein, vereinsleben=vereinsleben)
